...
# REQUIRED: Store result in dictionary 'promo_comparison'
...
top_idx = region_category_sales.groupby("Region")["Total_Sales"].idxmax()
top_categories_by_region = region_category_sales.loc[top_idx].set_index("Region")["Category"]
...
# REQUIRED: Store result in variable 'product_revenue_analysis'
product_revenue_analysis = sales_df.groupby("Product")["Total_Sales"].sum().sort_values(ascending=False).to_frame(name="total_revenue")
...
print("1. Increase focus on Smartphones, since they are the top-performing category in multiple regions and PhoneX is the highest revenue-generating product.")
print("2. Review promotion strategy, because promoted items generated substantial revenue overall, but their average sale value was lower than non-promoted items.")
print("3. Address missing data in Units and Unit_Price immediately, since incomplete values can reduce the accuracy of pricing, sales, and inventory analysis.")

# Optional extra outputs for verification
